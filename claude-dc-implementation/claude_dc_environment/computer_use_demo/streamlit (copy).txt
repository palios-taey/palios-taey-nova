...
def _render_api_response(
    ...
    response: httpx.Response | dict | object | None,
    response_id: str,
    tab: DeltaGenerator,
):
    """Render API request/response in the HTTP logs tab."""
    with tab:
        with st.expander(f"Request/Response ({response_id})"):
            ...
            if isinstance(response, httpx.Response):
                ...
                st.markdown(f"`{response['status_code']}`{newline}{newline.join(f'`{k}: {v}`' for k, v in response.get('headers', {}).items())}")
