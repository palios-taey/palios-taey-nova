...
log_file = '/tmp/token_manager.log'
logging.basicConfig(
    level=logging.INFO,
...
class TokenManager:
    ...
    def __init__(self):
        ...
        self.input_token_warning_threshold = 0.7  # 70% of input limit for more conservative rate limiting
        ...
        # Settings for maximized token usage (optimized for Claude 3.7 with 128k output beta)
        self.safe_max_tokens = 128000  # Maximum for 128k output beta
        self.safe_thinking_budget = 32000  # Enhanced thinking budget for complex tasks
    ...
    def check_output_rate_limit(self, output_tokens: int) -> Tuple[bool, float]:
        ...
        # Check if we're approaching the limit (70% of 16K = 11.2K)
        if self.output_tokens_per_minute >= self.org_output_limit * 0.7:
            # Calculate how long we need to wait for the oldest request to drop off
