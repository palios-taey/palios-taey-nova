"""
Streamlit interface for Claude Computer Use with streaming, tool use, and thinking capabilities.
"""
import os
import sys
import json
...
logger = logging.getLogger("claude_ui")
...
def handle_stream_event(event, response_container):
    """Handle a streaming event update"""
...
async def process_message(user_input, response_container, tool_container):
    """Process a user message and display the streaming response"""
    ...
    # Add the user message to the Claude history
    ...
    def handle_output(event):
        handle_stream_event(event, response_container)
    
    def handle_tool_output(tool_input, tool_id):
        with tool_container:
            st.subheader(f"Tool Output (ID: {tool_id})")
            st.json(tool_input)
...
def main():
    ...
    if st.session_state.submitted:
        ...
        if user_input:
            ...
            # Process message
            run_async(process_message(user_input, response_container, tool_container))
