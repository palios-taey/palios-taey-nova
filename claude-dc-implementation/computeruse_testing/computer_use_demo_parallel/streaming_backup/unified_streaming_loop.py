...
try:
    ...
    # Import XML system prompt if available
    try:
        from xml_system_prompt import XML_SYSTEM_PROMPT
        logger.info("Using XML-style system prompt for function calls")
        HAS_XML_SYSTEM_PROMPT = True
    except ImportError:
        logger.warning("XML system prompt not found, using default")
        HAS_XML_SYSTEM_PROMPT = False
except ImportError:
...
async def unified_streaming_agent_loop(
    ...
    api_params = {
        ...
        "system": XML_SYSTEM_PROMPT if HAS_XML_SYSTEM_PROMPT else DC_SYSTEM_PROMPT,
        "tools": tools,
        "stream": True
    }
    
    # Log which system prompt is being used
    if HAS_XML_SYSTEM_PROMPT:
        logger.info("Using XML system prompt for function calls")
    else:
    ...
    # Initialize the SimpleToolBuffer directly in this file
    class SimpleToolBuffer:
        """
        Simple buffer for accumulating partial JSON/XML for tool calls during streaming.
        ...
        def process_content_block_stop(self, index: int) -> Optional[Dict[str, Any]]:
            ...
            # Check for XML function calls first
            if '<function_calls>' in buffer and '</function_calls>' in buffer:
                # Extract tool name using simple regex
                ...
                if tool_name_match:
                    ...
                    # Return complete XML function call
                    return {
                        'type': 'complete_xml',
                        'tool_name': tool_name,
                        'tool_params': params,
                    ...
                    return {
                        ...
                        'error': 'Failed to parse XML function call',
                        'buffer': buffer,
                        'index': index
                    }
            ...
            try:
                ...
                # Return complete JSON function call
                return {
                    'type': 'complete_json',
                    'tool_name': tool_name,
                    'tool_params': params,
                ...
                # Not valid JSON or complete XML - check if it's partial XML
    ...
    logger.info("Using ToolCallBuffer for function call buffering")
        ...
        async for chunk in stream:
            ...
            if hasattr(chunk, "type"):
                ...
                if chunk_type == StreamEventType.CONTENT_BLOCK_START:
                    ...
                    # Handle tool use blocks (direct method)
                    elif block.type == "tool_use":
                        tool_name = block.name
                        tool_input = block.input
                        tool_id = getattr(block, "id", "tool_1")
                        
                        # Log that we're using the direct method (not the buffer)
                        logger.info(f"Using direct tool execution via content_block_start: {tool_name}")
                ...
                # Content block delta
                    ...
                    # Handle text content if not a tool call
                    if hasattr(chunk.delta, "text") and chunk.delta.text:
                        response_text_chunks.append(chunk.delta.text)
                        enhanced_callbacks.on_text(chunk.delta.text)
                    
                    # Handle tool calls using buffer pattern
                    elif hasattr(chunk.delta, "type") and chunk.delta.type == "input_json_delta":
                        # Process with buffer - accumulate partial JSON/XML
                        buffer_result = tool_buffer.process_content_block_delta(index=chunk.index, delta=chunk.delta)
                        
                        # Log for debugging
                        if buffer_result:
                            logger.info(f"Buffer processing delta: {buffer_result['type']} for index {chunk.index}")
                            
                        # We don't execute anything yet - just accumulate the content
                
                # Content block stop - very important for buffer pattern
                    ...
                    # Log buffer processing result
                    if buffer_result:
                        logger.info(f"Buffer processing stop result: {buffer_result['type']} for index {chunk.index}")
                        ...
                        if buffer_result["type"] == "complete_tool_call":
                            ...
                            tool_id = buffer_result["tool_use_id"] or f"toolu_{chunk.index}"
                            
                            logger.info(f"Complete tool call detected from buffer: {tool_name}")
                            
                            # Validate the tool parameters
                            is_valid, validation_message, validated_params = tool_buffer.validate_tool_parameters(tool_name, tool_params)
                            
                            if not is_valid:
                                # Tool parameters are invalid
                                logger.warning(f"Tool parameter validation failed: {validation_message}")
                                error_feedback = f"\n\n[Tool Parameter Error: {validation_message}]\n"
                            ...
                            # Log the tool use
                            logger.info(f"Added tool use to conversation history from buffer: tool_id={tool_id}, tool_name={tool_name}")
                            
                            # Execute the tool
                            tool_result, tool_result_content = await execute_streaming_tool(
                                tool_name=tool_name,
                                tool_input=validated_params,
                                tool_id=tool_id,
                                session=session,
                                enhanced_callbacks=enhanced_callbacks,
                                conversation_history=conversation_history
                            ...
                            try:
                                ...
                                # Log the conversation structure for debugging
                                logger.info(f"Resuming stream with {len(conversation_history)} messages after buffer-based tool call")
                                ...
                                logger.error(f"Error resuming stream after buffer-based tool call: {str(resume_error)}")
                                logger.error(traceback.format_exc())
                                await enhanced_callbacks.on_error(f"Error resuming stream: {str(resume_error)}", True)
                                
                        # Handle tool call errors from buffer
                        elif buffer_result["type"] == "tool_call_error":
                            logger.warning(f"Tool call error from buffer: {buffer_result['error']}")
                            # Provide feedback to help Claude DC improve the tool call
                            error_feedback = f"\n\n[Tool Call Error: {buffer_result['error']}]\n"
                            enhanced_callbacks.on_text(error_feedback)
                            response_text_chunks.append(error_feedback)
                            
                        # Handle partial tool calls - this is expected for incomplete function calls
                        elif buffer_result["type"] == "partial_tool_call":
                            logger.info(f"Partial tool call detected: {buffer_result.get('buffer', '')[:50]}...")
                            # Don't provide feedback to the user for partial calls
