"""
Fixed version of production_ready_loop.py with better error handling.
This version adds null checks and proper validation for API callbacks.
"""
...
async def sampling_loop(
    *,
    ...
    FIX: Added proper error handling and null checks for api_response_callback
    """
    ...
    try:
        ...
        # Pass the request if available, otherwise None
        request = getattr(e, 'request', None)
        api_response_callback(request, getattr(e, 'response', None), e)
        return messages
    except APIError as e:
        # Pass the request if available, otherwise None
        request = getattr(e, 'request', None)
        api_response_callback(request, getattr(e, 'body', None), e)
        return messages
    except Exception as e:
        # For general exceptions, we don't have request/response objects
