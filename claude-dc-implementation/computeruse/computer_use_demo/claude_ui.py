...
class ClaudeUI:
    ...
    def __init__(self):
        ...
        if "model" not in st.session_state:
            st.session_state.model = "claude-3-7-sonnet-20240425"
    ...
    def setup_ui(self):
        ...
        with st.sidebar:
            ...
            st.session_state.model = st.selectbox(
                "Model",
                ["claude-3-7-opus-20240229", "claude-3-7-haiku-20240307", "claude-3-7-sonnet-20240425"],
                index=2,
                key="model_input"
            )
        ...
        # Handle user input
        logger.info(f"Received user input: {user_input[:20] if user_input else 'None'}...")
        if user_input:
            ...
            logger.info(f"Stored user input in session state: {user_input[:20] if user_input else 'None'}...")
