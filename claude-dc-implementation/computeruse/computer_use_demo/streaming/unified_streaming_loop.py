...
# Simple buffer class for tool call accumulation
class SimpleToolBuffer:
    """
    Simple buffer for accumulating partial JSON/XML for tool calls during streaming.
    
    This implementation prevents the race condition where partial function calls
    are processed prematurely during streaming.
    """
    ...
    def extract_xml_function_call(self, xml_str: str) -> Dict[str, Any]:
        ...
        name_match = re.search(r'<invoke name="([^"]+)"', xml_str)
        if not name_match:
            result['error'] = "Missing function name in XML"
            return result
    ...
    def process_content_block_stop(self, index: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # If this is still partial JSON, keep in buffer
            if index in self.json_buffers:
                logger.debug(f"Partial JSON in buffer: {buffer}")
                return {
                    "type": "partial_tool_call",
                    "buffer": buffer,
                    "error": f"Incomplete JSON: {str(e)}",
                    "tool_use_id": self.tool_use_ids.get(index),
                    "is_complete": False,
                    "index": index
                }
            
            # Not valid JSON or XML, return error
            return {
                "type": "tool_call_error",
